...
def callback():
    ...
    try:
        ...
        reply_quick(line_bot_api,json_data)
...
def reply_quick(line_bot_api,json_data):
...
def postback_event(line_bot_api,json_data):
    ...
    if json_data['events'][0]['type'] =='postback':
        ...
        if value =='確定':
            print(reply_token)
            img_urls = pinterest.pin_search(driver,name,int(num))
            #判斷回傳是否字串
            if isinstance(img_urls,str):
                line_bot_api.reply_message(reply_token,TextSendMessage(text=img_urls))
            else:
                reply_img(line_bot_api,reply_token,img_urls)
        else:
...
def reply_img(line_bot_api,reply_token,img_urls):
    message = []
